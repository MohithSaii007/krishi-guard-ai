    ...
    <SlideLayout>
      <div className="grid h-full grid-cols-[42%_58%]">
        ...
        <div className="flex flex-col justify-center px-20 pb-32 pt-20">
          ...
          <h2 className="slide-title mt-6 max-w-[900px] text-forest">
            Farming decisions are still highly uncertain.
          </h2>
          <p className="slide-body mt-5 max-w-[840px] text-charcoal/70">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            {items.map(({ icon: Icon, label, text }, i) => (
              <div
                key={label}
                className="anim rounded-3xl border border-forest/10 bg-white/70 px-7 py-5 shadow-[0_10px_30px_-22px_var(--forest)]"
                style={{ animationDelay: `${i * 0.09}s` }}
              >
                <Icon className="size-8 text-leaf" strokeWidth={1.6} />
                <div className="slide-badge mt-3 font-semibold text-forest">{label}</div>
                <div className="slide-caption mt-2 text-charcoal/65">{text}</div>
              </div>
            ))}
          </div>
          <div className="mt-7 rounded-2xl border-l-4 border-gold bg-forest px-8 py-5">
