    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/auth" className="hidden rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 sm:block">Sign in</Link>
          <Link to="/auth" search={{ mode: "signup" }} className="hidden rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-600 sm:block">Create account</Link>
      ...
      {open && <nav className="border-t border-slate-100 bg-white px-5 py-4 md:hidden">
        ...
        <Link to="/auth" className="mt-2 block rounded-xl bg-slate-950 px-4 py-3 text-center text-sm font-semibold text-white">Sign in or create account</Link>
      </nav>}
    </header>
  );
}