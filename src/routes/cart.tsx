...
function CartPage() {
  ...
  function continueCheckout() {
    if (signedIn) navigate({ to: "/orders" });
    else navigate({ to: "/auth", search: { mode: "signup" } });
  }
  return <div className="min-h-screen bg-slate-50">
    <MarketplaceHeader />
    <main className="mx-auto max-w-6xl px-5 py-10">
      <Link to="/domainnest" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-indigo-600"><ArrowLeft className="size-4" />Continue searching</Link>
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="font-display text-xl font-bold text-slate-950">Order summary</h2><div className="mt-5 flex justify-between text-sm text-slate-600"><span>Domains ({items.length})</span><span>${subtotal.toFixed(2)}</span></div><div className="my-5 h-px bg-slate-100" /><div className="flex items-end justify-between"><span className="font-semibold text-slate-900">Total</span><span className="font-display text-2xl font-bold text-slate-950">${subtotal.toFixed(2)}</span></div><p className="mt-1 text-right text-xs text-slate-400">USD · Taxes calculated later</p>
          <button onClick={continueCheckout} disabled={!items.length} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3.5 text-sm font-bold text-white hover:bg-indigo-700 disabled:bg-slate-100 disabled:text-slate-400"><LockKeyhole className="size-4" />{signedIn ? "Continue to checkout" : "Sign in to checkout"}</button>
